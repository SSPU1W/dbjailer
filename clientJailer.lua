	...
	if DoesEntityExist(pP) then
		...
		Citizen.CreateThread(function()
			...
			while jT > 0 and not eJE do
				...
				if jT % 30 == 0 then
					TriggerEvent('chatMessage', 'SYSTEM', { 0, 0, 0 }, jT .." seconds until release.")
				end
				Citizen.Wait(500)
				...
				if D > 2 then -- distance#######################################################################################
					...
					if D > 4 then
						...
						TriggerEvent('chatMessage', 'JUDGE', { 0, 0, 0 }, "Jail time increased for attempted escape!")
					end
				end
				jT = jT - 0.5
			end
			TriggerServerEvent('JailRelease') -- UDPATE DB TO RELESE PLAYER
			...
			while jT > 0 and not eJE do
				...
				if D > 2 then
					...
					if D > 4 then
						...
						TriggerEvent('chatMessage', 'JUDGE', { 0, 0, 0 }, "Your time has been increased for attempted escape!")
					end
				end
				jT = jT - 0.5
			end
			...
			while jT > 0 and not eJE do
				...
				if D > 2 then
					...
					if D > 4 then
						...
						TriggerEvent('chatMessage', 'JUDGE', { 0, 0, 0 }, "Your time has increased for attempted escape!")
					end
				end
				jT = jT - 0.5
			end
			TriggerServerEvent('JailRelease') -- UDPATE DB TO RELESE PLAYER
			TriggerServerEvent('chatMessageEntered', "SYSTEM", { 0, 0, 0 }, GetPlayerName(PlayerId()) .." has been released from jail.")
			SetEntityCoords(pP, 432.95864868164, -981.41455078125, 29.710334777832)
			cJ = false
			SetEntityInvincible(pP, false)
		end)
